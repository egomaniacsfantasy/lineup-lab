# Olympus ESPN Connector

A tiny Manifest V3 browser extension that lets Olympus read your **private**
...
it to Olympus. It reads **only** `espn_s2` + `SWID` from `espn.com`, **only**
when the Olympus tab asks, and sends them **only** to that tab. Nothing is
stored or transmitted anywhere else.

## How it works

```
Olympus page  ──postMessage──▶  content.js  ──chrome.runtime──▶  background.js
                                                                  (chrome.cookies.get)
Olympus page  ◀──postMessage──  content.js  ◀──response────────  background.js
```

The content script only injects on the Olympus origin; the worker only ever
...
5. On Olympus, connect ESPN → "Sync with the extension."
